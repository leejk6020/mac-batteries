...
```sh
./tools/make_release_dmg.sh    # 아카이브 → Developer ID 서명 → 공증 → 스테이플 → DMG
```

최초 1회만 공증 자격증명을 저장하면 됩니다. [appleid.apple.com](https://appleid.apple.com)
에서 앱 암호를 발급받은 뒤:

```sh
xcrun notarytool store-credentials batterybar \
    --apple-id <Apple ID> --team-id NX4KU2A5JP
```

`--password` 를 생략하면 입력 프롬프트가 떠서 암호가 화면·셸 히스토리에 남지
않습니다. 이후 스크립트는 키체인에서 자동으로 읽어갑니다.

> **왜 `codesign` 직접 호출이 아니라 Xcode 아카이브를 거치나:**
> Xcode가 발급하는 Developer ID 인증서는 클라우드 관리형이라 개인 키가 로컬
> 키체인에서 검색되지 않습니다. CLI에서 `codesign --sign "Developer ID Application: …"`
> 을 호출하면 `no identity found` 로 실패합니다. 대신 아카이브 시
> `CODE_SIGN_ENTITLEMENTS=""` 로 샌드박스만 꺼서, 전체 기기를 지원하는 빌드를
> 얻습니다.
...
```sh
./build.sh              # 앱 빌드
./tools/make_dmg.sh     # dist/BatteryBar.dmg 생성
```

이쪽은 ad-hoc 서명이라 웹에서 다운로드하면 macOS가 격리 속성을 붙여 실행을
막습니다("확인되지 않은 개발자" 또는 "손상되었습니다"). DMG 안에 한/영/일
해제 안내가 동봉되지만, **공개 배포에는 쓰지 마세요.**
...
| `tools/make_release_dmg.sh` | 공증된 배포용 DMG 생성 (Developer ID 서명 + notarize + staple) |
